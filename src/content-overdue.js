...
export function createOverdueContent() {
  ...
  const overdueTasks = tasks.filter(task => {
    ...
    return isPast(date) && !isToday(date) && task;
  });

  // Get number of tasks
  function numberOfOverdueTasks() {
    const overdueTasksBtn = document.querySelector('button.overdue-tasks')
    const overdueTasksNumber = createOverdueContent().overdueTasks.length;
    const overdueTasksNumberNode = document.createTextNode(overdueTasksNumber);
    const overdueTasksNumberWrap = document.createElement('span');
    overdueTasksNumberWrap.classList.add('total-tasks');
    overdueTasksNumberWrap.appendChild(overdueTasksNumberNode);
    overdueTasksBtn.appendChild(overdueTasksNumberWrap);
  ...
  tasksList.classList.add('tasks-list', 'overdue-tasks');
  ...
  overdueTasks.forEach((task, index) => {
    ...
    metadataWrap.classList.add('metadata', 'overdue');
    const date = new Date(task.dueDate);
  ...
  return { tasksList, overdueTasks, numberOfOverdueTasks };
}